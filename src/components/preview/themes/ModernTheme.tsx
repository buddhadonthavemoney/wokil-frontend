          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div className="animate-in fade-in slide-in-from-left duration-1000">
              <div className="inline-block px-4 py-1.5 bg-blue-500/10 border border-blue-500/20 rounded-full text-blue-400 text-sm font-semibold mb-8 tracking-wide uppercase">
                {profile.professionalTitle || 'Legal Professional'}
              </div>
              ...
              {profile.lawFirmName && (
                <p className="text-slate-400 text-2xl mb-4 font-light italic">{profile.lawFirmName}</p>
              )}
              ...
              <div className="flex flex-wrap gap-4">
                <a href={`tel:${profile.phoneNumber}`} className="px-8 py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold transition-all hover:scale-105 shadow-lg shadow-blue-500/25 flex items-center gap-2">
                  <Phone className="w-5 h-5" />
                  Contact Me
                </a>
                <a href={`mailto:${profile.email}`} className="px-8 py-4 bg-slate-900 hover:bg-slate-800 border border-slate-700/50 rounded-xl font-bold transition-all hover:scale-105 flex items-center gap-2">
      ...
      <main className="container mx-auto px-6 py-24">
        ...
        <section className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-24 anim-in fade-in slide-in-from-bottom duration-700 delay-700">
          <div className="p-8 bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-2xl group hover:bg-slate-900 transition-colors">
            ...
            <p className="text-xl text-white font-medium leading-none">{profile.phoneNumber || 'Phone number'}</p>
          </div>
          <div className="p-8 bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-2xl group hover:bg-slate-900 transition-colors">
            ...
            <p className="text-xl text-white font-medium break-all leading-tight">{profile.email || 'Email address'}</p>
          </div>
          <div className="p-8 bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-2xl group hover:bg-slate-900 transition-colors">
            ...
            <p className="text-xl text-white font-medium leading-snug">{profile.officeAddress || 'Office address'}</p>
          </div>
          <div className="p-8 bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-2xl group hover:bg-slate-900 transition-colors">
            ...
            <p className="text-xl text-white font-medium leading-none">{profile.officeHours || 'Office hours'}</p>
          </div>
        </section>
        ...
        {(profile.website || profile.linkedIn) && (
          <section className="flex flex-wrap gap-4 justify-center items-center py-12 border-y border-slate-800/50">
            {profile.website && (
              <a href={profile.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-6 py-3 bg-slate-900 border border-slate-800 rounded-full hover:border-blue-500/50 hover:text-blue-400 transition-all font-medium">
