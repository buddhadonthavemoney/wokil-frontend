...
import { getLegalResearchScopeOptionsOptions } from '@/generated/wokil-api/@tanstack/react-query.gen';
...
export function ScopePicker({ scope, onChange, disabled }: ScopePickerProps) {
  ...
  const optionsQuery = useQuery({
    ...getLegalResearchScopeOptionsOptions(),
    ...
    <div className="relative">
      ...
      {!isWholeCorpus(scope) && (
        <button
          type="button"
          onClick={() => onChange({})}
          className="relative z-20 ml-1.5 text-xs text-muted-foreground hover:text-foreground"
