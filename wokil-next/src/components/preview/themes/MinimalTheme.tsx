...
export function MinimalTheme({ profile }: MinimalThemeProps) {
  ...
  const fullName = basicInformation.fullName || 'Professional Presence';
  const professionalTitle = basicInformation.professionalTitle || 'Legal Counsel';
        ...
        <div className="max-w-4xl">
          ...
          <div className="grid md:grid-cols-2 gap-12 items-start">
            <p className="text-2xl text-[#5C564E] leading-relaxed font-light italic">
              {bio || 'Architecting legal solutions with precision and integrity.'}
            </p>
            <div className="flex flex-wrap gap-4 pt-4">
              <a href={`mailto:${email}`} className="inline-flex items-center gap-3 px-10 py-5 bg-[#1A1816] text-[#F9F8F6] rounded-full hover:bg-[#2D2A26] transition-all hover:scale-105 active:scale-95 font-bold text-sm tracking-widest uppercase shadow-xl shadow-[#1A1816]/10">
                Initiate Contact
          ...
          <aside className="lg:col-span-4 space-y-16">
            ...
            <div className="space-y-10 bg-[#F2EDE4] p-10 rounded-[2.5rem] border border-[#E5E1DA]">
              ...
              <div className="space-y-8">
                <div className="space-y-1">
                  ...
                  <a href={`tel:${phoneNumber}`} className="text-lg font-medium text-[#1A1816] hover:text-stone-500 transition-colors block leading-tight">
                    {phoneNumber || 'Available on request'}
                  </a>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] text-[#8C857D] font-bold uppercase tracking-widest">Email</p>
                  <a href={`mailto:${email}`} className="text-lg font-medium text-[#1A1816] hover:text-stone-500 transition-colors block break-all leading-tight">
                    {email || 'Professional inquiry'}
                  </a>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] text-[#8C857D] font-bold uppercase tracking-widest">Office</p>
                  <p className="text-lg font-medium text-[#1A1816] leading-snug">{officeAddress || 'Global Chambers'}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] text-[#8C857D] font-bold uppercase tracking-widest">Availability</p>
                  <p className="text-lg font-medium text-[#1A1816] leading-tight">{officeHours || 'By Appointment'}</p>
                </div>
              </div>
            </div>
            ...
            {(website || linkedIn) && (
              <div className="flex gap-4">
                {website && (
                  <a href={website} target="_blank" rel="noopener noreferrer" className="w-14 h-14 bg-[#1A1816] text-[#F9F8F6] rounded-full flex items-center justify-center hover:scale-110 transition-transform">
                    <Globe className="w-6 h-6" />
                  </a>
                )}
                {linkedIn && (
                  <a href={linkedIn} target="_blank" rel="noopener noreferrer" className="w-14 h-14 bg-[#1A1816] text-[#F9F8F6] rounded-full flex items-center justify-center hover:scale-110 transition-transform">
